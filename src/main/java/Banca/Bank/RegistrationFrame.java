...
public class RegistrationFrame extends JFrame {
    ...
    public RegistrationFrame(Banca bank) {
        this.bank = bank;
        initializeUI();
    }
    ...
    private void initializeUI() {
        ...
        mainPanel.setBackground(new Color(240, 242, 245));//Background
        ...
        formPanel.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createLineBorder(new Color(200, 200, 200)),//Border color of the text box panel
    ...
    private JTextField createFormField(String labelText, JPanel panel) {
        ...
        panel.add(Box.createRigidArea(new Dimension(0, 10))); //padding between the text fields
    ...
    private void registerUser() {
        ...
        if (usernameExists(username)) {
        	//Make the text field's border red
            JOptionPane.showMessageDialog(this, 
        ...
        if (strength.equals("Weak")) {
            ...
            if (confirm != JOptionPane.YES_OPTION) return;// "return" --> what does it do?
        }
        
        // Parse optional fields with default values
        double walletBalance = walletText.isBlank() ? 0.0 : parseDouble(walletText); //using ternary operators
        ...
        double salary = salaryText.isBlank() ? 0.0 : parseDouble(salaryText);
        ...
        LocalDate date = LocalDate.now();//.format(DateTimeFormatter.ofPattern("DD"));
        String time = LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm:ss"));
        //use the new refined method to create the String ("Methods")
        String s = username + " | " + password + " | " + name + " | " + surname + " | " + walletText + " | " + salaryText + " | " + "0.0" + " | " + date + " | " + '0' + " | " + username + "History.csv";
        
        // Create user account
        //boolean success = createUserAccount(name, surname, username, password, walletBalance, salary);
    ...
    //Check if it needs to be refined
    ...
    private Color getStrengthColor(String strength) {
        switch (strength) {
            case "Very Strong": return new Color(0, 150, 0);
            case "Strong": return new Color(0, 180, 0);
            case "Medium": return new Color(255, 165, 0);
    ...
    //Fix it and move it to "Methods"--> that's it ^.^
    /*private boolean createUserAccount(String name, String surname, String username, 
                                    String password, double walletBalance, double salary) {
        try {
            // Create user file
            File userFile = new File(username + ".csv");
            if (userFile.createNewFile()) {
                // Create history file
                new File(username + "History.csv").createNewFile();
                ...
                try (PrintWriter writer = new PrintWriter(new FileWriter(userFile))) {
                    ...
                    writer.println("wallet," + walletBalance);
                    writer.println("salary," + salary);
                    writer.println("bankBalance,0.0");
                    writer.println("initialDate," + LocalDate.now());
                    writer.println("investments,0");
                }
                
                // Create user object in bank
                // (Assuming you have appropriate classes)
                //Portafoglio wallet = new Portafoglio(walletBalance);
                //Account account = new Account(username, password, 0.0, LocalDate.now());
                //Utente user = new Utente(name + " " + surname, username, password, wallet, account, salary);
                //bank.addUser(user);
    ...
    //public static void main(String[] args) {
        // Test the registration frame
       // SwingUtilities.invokeLater(() -> {
            //Banca bank = new Banca("MyBank");
