...
public class Methods {
	...
	//Works (Check if it returns null)
	public static Banca parseBanca (String s) {
		//System.out.println("Entered parseBanca");
		...
		try (FileReader reader = new FileReader(s); 
			...
			if (input.hasNextLine()) {
				riga = input.nextLine();//ignoring the first line (I don't know if it's the correct way though.)
				if (input.hasNextLine()) {
					riga = input.nextLine();
					try {
	...
	public static boolean updateFile (String fileName, String replacement, int nLine, int field) {
		...
		try (FileReader reader = new FileReader(fileName.trim()); 
				...
				if (nLine > 0) {
					...
					//tryDeletingFile (oldFile);
					File newFile = new File ("new.csv");
					//newFile.renameTo(new File(fileName));
					System.out.println(newFile.renameTo(new File(fileName)));
	...
	/*public static void tryDeletingFile (File oldFile) {
	...
	/*public static void tryRenamingFile (Path source, Path destination, StandardCopyOption.REPLACE_EXISTING) {
	...
	public static boolean updateInvestments (String fileName, int from, String s/*, int to*/) {//s is generic
	...
	//Invoke it in the MainFrame //Can't just return "" when we don't have anything (spacing problem)
	...
	//Delete it. Write a method that will take a delimiter as parameter and a Vector of String. Then using a for it will build the string and return it.Can be used in this method, initialLines, in the RegistrationFrame to get the string using all the fields. 
	...
	public static String[] initialLines () {
		...
		lines[0] = "Username | Password | Name | Surname | Portafoglio | Salary | ContoBancario | InitialDate | Number of investments | File";
	...
	//no need to read the file as it'll be created if the username doesn't exist
	...
	public static void main (String[] args) {
		...
		//String s = "100 | 85 | 9 | 2025-04-03 | 0.10 | 95";
		
		//Investment in = parseInvestment (s);
		//System.out.println(in.toString());
		
		//Banca banca = parseBanca ("anya.csv");
		//System.out.println(banca.toString());
		//System.out.println(updateHistoryFile("anyaHistory.csv", "Deposited"));
		//System.out.println(updateHistoryFile("anyaHistory.csv", "Withdrawn"));
		
		//System.out.println(updateFile ("anya.csv", "hello1", 2, 2));
		/*String s1 = updateField (s, "hello", 4);
		if (s1 != null) {
